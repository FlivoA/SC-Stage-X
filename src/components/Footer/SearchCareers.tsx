import React from "react";

const SearchCareers: React.FC = () => {
  const roles = [
    "Security Research Analysts",
    "Governance Advisors",
    "Certification Framework Specialists",
    "Risk Assessment Consultants",
  ];

  return (
    <div className="min-h-screen bg-white text-gray-900 px-6 py-16">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-4xl font-semibold mb-6">
        Search Careers
        </h1>

        <p className="text-lg text-gray-600 leading-relaxed mb-10">
          Security Council seeks professionals committed to analytical rigor
          and institutional governance excellence.
        </p>

        <h2 className="text-2xl font-medium mb-4">
          Potential roles include:
        </h2>

        <div className="space-y-4 mb-12">
          {roles.map((item, index) => (
            <div
              key={index}
              className="border border-gray-200 rounded-xl p-5 bg-gray-50"
            >
              {item}
            </div>
          ))}
        </div>

        <div className="pt-6 border-t border-gray-200">
          <p className="text-gray-600 leading-relaxed">
            All positions emphasize structured thinking, neutrality, and
            evidence-based evaluation.
          </p>
        </div>
      </div>
    </div>
  );
};

export default SearchCareers;